...
Actions rather than as an always-on server. Checks every open metrics market
once, resolves + auto-pays anything that's finished, then exits.
...
def process_market(w3: Web3, contract, account, market_id: int) -> None:
    # struct order matches MetricsMarket.getMarket(): question, close_time, outcome,
    # yes_pool, no_pool, platform, username, video_id, metric_type, target_value
    (question, close_time, outcome, yes_pool, no_pool,
     platform, username, video_id, metric_type, target_value) = contract.functions.getMarket(market_id).call()
    ...
    try:
        measured_value = metrics_client.fetch_metric(platform, username, video_id, metric_type)
    except Exception as e:
        print(f"  [market {market_id}] {question} — fetch failed: {e}, will retry next run")
        return

    outcome_to_set = OUTCOME_YES if measured_value >= target_value else 2  # 2 = No
    print(f"  [market {market_id}] {question} — measured {measured_value} vs target {target_value}. Resolving.")
